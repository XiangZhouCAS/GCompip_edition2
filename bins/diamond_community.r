...
option_list <- list(
  ...
  make_option(c("--result","-o"),type = "character",default = F,
              help = "Please set the name for the result file."),
  ...
  make_option(c("--diamond_db","-d"),type = "character",default = F,
              help = "Please specify the directory for the single-copy genes DIAMOND database."),
  ...
  make_option(c("--min_length","-m"),type = "numeric",default = 90,
	      help = "Set the minimum length required for filtering reads, the default is 90, but it is recommended to set this parameter to 140 if hydrogenases or hydrogen metabolism terminal enzymes are to be calculated."),
  ...
  make_option(c("--filter_condition","-l"),type = "character",default = "default",
	      help = "Please specify the filter_condition file. By default, the identity is set to 50 and the coverage to 80."),
...
opt_parser = OptionParser(
  usage = "usage: comts custom diy [options]",
  option_list = option_list,
  add_help_option = TRUE,
  prog=NULL ,
  description = "This Script is to calculate single copy gene abundance in microbial community (GAM,%).")
...
singleM_out <- paste0(basename(outpath),".UCSG.hits.txt")
...
s_out <- paste0(basename(outpath),".UCSG.hits.comb.txt")
