...
option_list <- list(
  ...
  make_option(c("--UCSG_db","-u"),type = "character",default = F,
              help = "Please specify the directory for universal single-copy genes database (e.g., 'Ribo_14.dmnd'). If you have already calculated the RPKM for these genes, you may instead specify the directory for the results (e.g., 'sample_name.singleM.hits.txt') to skip this step."),
...
singleM_out <- paste0(basename(outpath),".singleM.hits.txt")
